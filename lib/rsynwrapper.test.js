...
test('syncs a single file with spaces and dash in filename', done => {
	const rsync = require('../lib/rsyncwrapper')
	rsync({
			src: 'fixtures/file - spaces.txt',
			dest: 'tmp/',
		},
		err => {
			expect(err).toBeNull()
			expect(existsSync('tmp/file - spaces.txt')).toBe(true)
			done()
		}
	)
})
...
test('handles pre-escaped paths with dash and spaces', done => {
	const rsync = require('../lib/rsyncwrapper')
	rsync({
			// prettier-ignore
			src: 'fixtures/file\ -\ spaces.txt',
		...
		err => {
			expect(err).toBeNull()
			// prettier-ignore
			expect(existsSync('tmp\ with\ spaces/file\ -\ spaces.txt')).toBe(true)
			done()
		}
	)
})
